...
│   ├── index.ts             # CLI 入口（注册表模式，33 条命令）
│   ├── commands/            # 33 条命令（每文件 ≤300 行）
│   ├── lib/                   # 27 核心模块 + 10 子目录
│   │   ├── cli.ts              # CLI 参数安全解析
...
│   ├── __tests__/            # 41 文件，353 测试（含 fixture-gen 10 个测试文件, 73 个测试）
...
| `rules/project/coding/testing.md` | 5 测试层 | 5/5 | 353+73 tests, 0 fail |
...
npx tsx --test __tests__/*.test.ts # 353+73 tests（47 基础 + 10 fixture-gen）
```
