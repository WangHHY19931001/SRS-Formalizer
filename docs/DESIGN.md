...
```
Frontend → _ctx/shard_index.json, ir/**/*.jsonl, srs-ir.json
Middle-end → middle-end/**/*.json, srs-ir.json (只读分析副本)
Backend draft → outputs/bdd/draft/*.feature,
                outputs/tlaplus/draft/*.tla + *.cfg,
                outputs/lean4/draft/*.lean
Backend verified → outputs/bdd/verified/*.feature,
                   outputs/tlaplus/verified/*.tla + *.cfg + validation/*.json,
                   outputs/lean4/verified/**/*.lean + validation/*.json
Backend deterministic → outputs/graphs/*.cypher + *.json,
                        outputs/fixtures/**, outputs/reports/**
```

草稿目录与 verified 目录必须物理隔离。只有对应严格验证成功、验证报告包含输入 source hash、工具版本、执行时间和通过结论时，产物才可由 draft 迁入 verified。FINAL、交付清单、跨图验证和执行上下文只读取 verified 与确定性产物。
...
| FINAL | R3 检查 + `validate-bdd --strict`、全部 verified TLA+ 的 `validate-tla --strict`、全部 verified Lean 4 的 `validate-lean --strict`、图谱与追溯产物、全部 checklist、NFR 覆盖率 ≥ 80% |

FINAL 只能读取 `outputs/**/verified/` 与确定性产物。若 IR 要求 BDD/TLA+/Lean，而对应 verified 产物不存在、工具链未运行或验证报告缺失，则 FINAL 必须失败；不得以“未触发”、空目录、草稿文件或弱化文本检查视为通过。
...
- 清理旧 `.stl`、`.tlc`、checkpoint/trace 状态后，按 SANY → TLC 的顺序验证。
...
`validate-lean --strict` 必须递归检查完整 proof project，而不是只检查传入的单个文件。失败必须返回 `{ status: "error" }` 和非零退出；只有通过后才可迁入 verified。
