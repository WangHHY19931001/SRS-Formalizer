...
> **版本**: 2.1.0 | **日期**: 2026-07-21 | **状态**: Active
>
...
| `validate-dataflow` | 数据流抽取 JSONL | entity/flow 记录格式：id 形状（`DE-<slug>` / `R[123]-…`）、无悬挂 entity_id、无重复 id、action 枚举。纳入 S1 门禁（缺失则 PASS，抽取可选） | `--file <path> --workdir <path>` |
...
| 命令 | 类别 | 功能 | 关键参数 |
|------|------|------|----------|
| `assemble-ir` | 数据结构装配 | 读取所有 JSONL（含 `data-entities/` 数据流抽取）→ 去重 → 装配 `srs-ir.json` + 引用完整性校验（悬挂边/重复 ID/版本号 `2.x`/buildTimestamp 非空）；数据流记录按 canonical 归一为 `data_entity` 节点 + `produces`/`consumes`/`mutates` 边。不分析、不发射、不修改 JSONL | `--workdir <path>` |
| `check-connectivity` | 图算法 | 跨 shard 连通性、强连通分量（SCC）、孤岛检测、桥接边建议。LLM 在大图上无法可靠执行 | `--workdir <path>` |
| `analyze-dataflow` | 图算法（只读旁路） | 只读 IR，检出四类数据流可疑单元：dead_data / gap / boundary / cycle → `3_graph/analysis/dataflow.json`。恒 warning，不 fail-closed；无 `data_entity` 降级为空。`--assess` 写注入门控 | `--workdir <path> [--assess --fp-rate <r> --sample-size <n> --assessed-by <name>]` |
...
```
.srs_formalizer/
├── srs-ir.json            # 核心中间表示 (v2.1.0)，assemble-ir 产出
...
│   ├── r3-relational/
│   ├── architecture/
│   └── data-entities/     # F4e: 数据流抽取 JSONL (entity/flow, 2.1.0)
...
| F4e | 数据流抽取（从[已提取需求+glossary+文档]识别数据实体与读写关系；entity/flow 记录，canonical 归一；抽取铁律守 Inversion，臆造挂 GAPS） | `2_extract/data-entities/*.jsonl` | `validate-dataflow --file` + S1 格式门禁 |
| F5 | 调用工具装配 IR → 层次交联需求图谱（含 data_entity 节点 + 数据流边） | `srs-ir.json` + `graph.merged.json` | `assemble-ir --workdir` |
...
| M1.5 | 读 IR → 数据流四类检出（dead_data/gap/boundary/cycle）→ 强提示（warning，非硬门禁） | `3_graph/analysis/dataflow.json` | `analyze-dataflow --workdir` |
...
```typescript
interface SRSIR {
  version: '2.0.0' | '2.1.0';  // 2.1.0 新增数据流节点/边，向后兼容 2.0.0
  meta: IRMeta;
  nodes: IRNode[];
  edges: IREdge[];
  crossRefs: CrossRef[];
...
type IRNodeType =
  ...
  | 'data_entity';  // 2.1.0 数据流：Frontend F4e 抽取，Middle-end M1.5 分析
...
type IREdgeType =
  ...
  | 'produces' | 'consumes' | 'mutates';  // 2.1.0 数据流边，方向 requirement → data_entity
...
- **数据流节点/边（`data_entity` + `produces`/`consumes`/`mutates`，2.1.0）由 Frontend F4e 抽取**：Agent 产出 `2_extract/data-entities/*.jsonl`（entity/flow 记录），`assemble-ir` 按 canonical 归一写入 IR。Middle-end M1.5 `analyze-dataflow` 只读消费、只写 `3_graph/analysis/dataflow.json`，不改 IR（对齐 ADR-0009）
...
Frontend → _ctx/shard_index.json,
           2_extract/{r1-explicit,r2-implicit,r3-relational,architecture,data-entities}/**/*.jsonl,
           srs-ir.json (assemble-ir 产出)
Middle-end → 3_graph/{graph,analysis}/**/*.json (含 analysis/dataflow.json),
...
**R3 原子操作树闸门**（收敛判据·建模完整性）：`check-connectivity` 在输出中附带 `atomicTree` 报告（`analyzeAtomicTree`），把系统建模抽象为一棵**以顶层系统为根、沿 `contains` 边逐层展开子系统、叶子挂载原子需求的树**。这比无向连通分量更强，专抓"连通但不成树"的病态建模。R3 门禁 `checkAtomicTree` 要求良构（`wellFormed`）——五项全满足才通过，任一违反即失败并给出对应字段明细：

| 字段 | 判据 | 违反含义 |
|------|------|----------|
| `roots` | 无入向 `contains` 的 architecture 节点须**恰 1 个** | 0 个=成环无根；>1=多顶层系统未收敛 |
| `cyclicContains` | architecture 间 `contains` 无环（三色 DFS） | 成环则不构成树 |
| `unreachableArchitecture` | 从根沿 `contains` BFS 须覆盖全部 architecture 节点 | 非空=游离子系统（建模断裂） |
| `emptyLeafSubsystems` | 叶子子系统须挂载 ≥1 原子需求（经 `contains`/`refines`/`traces_to`/`implements`/`derived_from` 边） | 非空=空壳状态机，无原子操作可执行 |
| `uncoveredRequirements` | 每条 requirement 须经上述 attach 边挂进树 | 非空=游离需求（未归属任何子系统） |

**数学与 FSM 语义**：此判据本质是判断 `contains` 关系是否构成一棵**以顶层系统为根的有向生成树/森林**。它同时是**多层有限状态机抽象的静态骨架**——每个 architecture 节点对应一台（子）状态机，`contains` 是层次精化（refinement）关系，叶子子系统是执行原子操作的最底层状态机，逐层组装即构成顶层系统状态机。良构的树随 IR 注入 Backend B2/B3：BDD 按叶子子系统划 Feature 边界、按根到叶路径生成端到端功能链；TLA+ 按 `contains` 深度做 L1（根）→L2（中间层）→L3（叶子）层次化 module 拆解（见 `executor-bdd.md`/`executor-tlaplus.md` 的「原子操作树」章节）。无 architecture 节点时跳过（判据不适用）。
...
**处理**：
1. 读取 `2_extract/{r1-explicit,r2-implicit,r3-relational,architecture,data-entities}/**/*.jsonl`
2. 按 `id` 去重
3. 构建 `IRNode[]` + `IREdge[]`；数据流记录（`data-entities/`）经校验后按 canonical 归一为 `data_entity` 节点 + `produces`/`consumes`/`mutates` 边
4. 合并 `crossRefs`、`nfrProfile`、`gaps`、`glossary`（来自 shard_index.json）
5. 完整性验证：版本号 `2.x`（产出 `2.1.0`，接受 `2.0.0`）、无悬挂边（`source`/`target` 必须在 `nodes[]` 中存在）、`buildTimestamp` 非空
...
| 版本 | 日期 | 关键变更 |
|------|------|----------|
| 2.1.0 | 2026-07-21 | **数据流审视提示**（spec 2026-07-21 / ADR-0009）：SRS-IR 扩展 `data_entity` 节点 + `produces`/`consumes`/`mutates` 边（向后兼容 2.0.0）。Frontend F4e 抽取数据实体与读写关系（`executor-frontend-dataflow.md` → `2_extract/data-entities/*.jsonl`，canonical 归一），Middle-end M1.5 `analyze-dataflow` 只读检出四类可疑单元（dead_data/gap/boundary/cycle）→ `dataflow.json`，恒 warning 不 fail-closed。新增命令 `validate-dataflow`（格式门禁，纳入 S1）+ `analyze-dataflow`（分析 + `--assess` 注入门控）。层次 2 executor 注入默认 shadow 模式，经假阳性率评估签署后放开。命令数 17→19 |
