...
- **所有系统严禁死锁**。死锁是系统设计缺陷的直接证据，必须修正根因，不得绕过。
- **严禁通过禁用死锁检测来规避死锁**：禁止在 TLC 命令行传入 `-deadlock` 标志（该标志会关闭死锁检测，使 TLC 不再将无后继状态判为错误）。任何试图通过关闭死锁检测来"通过"验证的行为，与未做形式化验证等价，视为重大违规。
- 遇到死锁时的唯一合法处理路径：利用 TLC 生成的反例轨迹定位死锁发生的具体状态，分析缺失的状态转换，修正 `.tla` 模型或（若根因在 SRS 设计层）写入 `SRS_PATCHES.md` 并等待人工确认。
- 调试前先删除旧的轨迹文件（`.stl`）和状态文件（`.tlc`），确保验证环境干净。
- 编码顺序：先通过 SANY 语法检查，再执行 TLC 模型检查。

### 3.5 质量标准

不允许：死锁、通过禁用死锁检测绕过死锁、状态爆炸、违反不变式、实现错误、占位实现、简化实现、错误实现。
...
- [ ] SANY 语法检查通过？
- [ ] TLC 模型检查通过？
- [ ] 无死锁（TLC deadlock freedom 通过，**禁止**使用 `-deadlock` 标志绕过）？
- [ ] 无状态爆炸？
- [ ] TypeOK 不变式覆盖所有变量？
- [ ] 至少一个正确性不变式？
- [ ] 注入 Bug 能被捕获？
