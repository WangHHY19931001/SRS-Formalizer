...
### 3.8 lake build 流程（三步构建，必须按顺序）

> ⚠️ **跳过 `lake exe cache get` 会导致从源码编译整个 Mathlib（数小时），
> 这是 lake build 卡住的最常见原因。**

构建 Lean 4 项目必须按以下三步顺序执行，缺一不可：

1. `lake update` — 拉取 mathlib4 依赖，更新 `lake-manifest.json`
2. `lake exe cache get` — **下载预编译 `.olean` 缓存（关键步骤！必执行）**
   - 跳过此步：从源码编译整个 Mathlib，耗时数小时，lake build 会长时间卡住
   - 执行此步：下载社区预编译产物，`lake build` 只需编译用户自己的 `.lean` 文件（秒级到分钟级）
3. `lake build` — 只编译用户自己的 `.lean` 文件

每次修改 `.lean` 文件后立即 `lake build`，不要积攒多个修改一起编译。
首次克隆项目或更新 mathlib4 依赖后，必须重新执行 `lake exe cache get`。

### 3.8.1 lean-toolchain 版本一致性
