...
| 检查项 | 要求 |
|------|------|
| **无死锁（硬性，不可绕过）** | Deadlock freedom——每个可达状态必须有至少一个后继状态。**严禁**通过 `-deadlock` 标志禁用死锁检测来规避此项。死锁必须修正根因，不得压制检测。 |
...
> **死锁零容忍原则**：任何系统均不允许存在死锁状态。`-deadlock` 标志（关闭死锁检测）在本技能中被永久禁用——使用该标志的验证报告不得通过门禁，视同未做死锁验证。
