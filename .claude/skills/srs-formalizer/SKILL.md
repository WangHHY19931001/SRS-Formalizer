...
post_conditions:
  ...
  - "verify-gate FINAL 全部通过，或 STATE.md 明确记录用户裁剪与残余风险"
permissions:
  - kind: filesystem
    ...
    read_only: false
  - kind: execute
    scope: "npx tsx .claude/skills/srs-formalizer/scripts/index.ts <registered-command>"
    description: Only registered commands through index.ts
    read_only: false
fallbacks:
  - operation: "文件写入"
    strategy: "使用工作目录内的原子临时文件与 rename；失败时保留上一次有效产物。"
  - operation: "工具链不可用"
    strategy: "写入 S5_SKIP_REPORT.md，记录受影响需求、替代验证与残余风险；不得伪造 verified。"
input_schema:
  type: object
  ...
  properties:
    ...
    stages: { type: array, items: { type: string, enum: [frontend, middle-end, backend] } }
output_schema:
  type: object
  properties:
...
metadata:
  ...
  post_conditions:
    ...
    - "cross-graph-report.json 必须存在且 13 个问题均有结论"
    - "convergence-log.jsonl 必须记录实际执行轮次"
    - "用户裁剪阶段时，STATE.md 必须记录跳过阶段、原因与残余风险"
  permissions:
    - kind: filesystem
      scope: ".srs_formalizer/*"
      description: All workflow outputs limited to working directory
  ...
  output_schema:
    ...
    properties:
      ...
      tlaplus:
        type: object
        description: "TLA+ 形式化规约（默认全模块覆盖；用户明确裁剪时必须记录跳过范围与风险）"
      lean4:
        type: object
...
**Gate Validators（12 个，只读校验，绝不产生语义产物）**：

| 命令 | 何时调用 |
|------|----------|
| `validate-jsonl --file <path> --workdir <wd>` | F2/F4 每个提取批次后，校验 JSONL 6 项（id 正则/category/confidence/statement/source_file/metadata） |
| `validate-shard-index --workdir <wd>` | F1 分片后，校验 ShardIndex schema（version/source_path/shard id/locator/行号） |
...
以下反模式来自实际执行中的常见错误，Agent 在驱动管线时必须主动规避：

| 反模式 | 后果 | 正确做法 |
|--------|------|---------|
| **Agent 自行提取/推导需求**（不派子代理，直接读 SRS 写 JSONL） | 引入主观偏差与格式错误，导致 `validate-jsonl` 批量失败 | 必须经 `prompts/executor-frontend-parse.md` 派子代理提取；Agent 只做分派与校验 |
| **跳过门禁直接阶段提升**（未跑 `verify-gate` 或 `validate-*` 就进入下一阶段） | 错误向下游放大，Backend 基于脏数据生成不可行形式化产物 | 每阶段末尾必须调用对应门禁，通过后才可进入下一阶段 |
| **同一轮次修改多个 Backend 产物**（同时改 TLA+、Lean、BDD） | 失败时无法归因，回滚成本高 | 每轮只提升一个产物，验证通过后再处理下一个 |
| **手动创建 `.cfg` 或 Lake 项目结构** | 结构与 `.tla`/`.lean` 不匹配，SANY/TLC/`lake build` 报错 | 由 Agent 生成草稿 → `validate-* --strict --promote` 自动校验提升 |
| **消费 draft 产物进行跨图验证** | `verify-gate FINAL` 因 `sourceHash` 不匹配或 `lifecycle: draft` 拒绝 | 只消费 `verified/` 目录产物，且 hash 必须匹配当前内容 |
| **收敛循环无限加轮不经人类确认** | 资源耗尽，且可能陷入局部震荡 | 严格按规模自适应上限执行；超限必须 🛑 **STOP** 等待人类确认 |
| **子代理 ID 含中文或不符合正则** | `validate-jsonl` 硬拒绝，整批提取作废 | 强制使用 `^R[123]-[A-Za-z0-9_.]+-\d{4}$` 格式，Agent 生成前校验 |
| **在 Windows 环境强制运行 Lean 4** | 工具链可能不可用，反复重试浪费时间 | 先探测 `lake --version`；不可用则生成 `S5_SKIP_REPORT.md`，记录替代验证与残余风险，且不得标记 Lean verified |
...
> **Agent 注意**：所有命令必须通过 `npx tsx index.ts <command>` 调用。参数值禁止使用 `undefined`、`null`、`NaN` 等占位符。所有 24 命令清单以 `index.ts` 注册表为唯一来源。
