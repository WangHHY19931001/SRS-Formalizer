...
## 原子操作树报告注入（B2/B3 建模辅助）

Middle-end M4 的 `check-connectivity` 输出含 `atomicTree` 报告（`3_graph/analysis/connectivity.json`）——以顶层系统为根、沿 `contains` 逐层展开子系统、叶子挂载原子需求的建模完整性判据，同时是多层有限状态机抽象的静态骨架。编排者在分派 B2（BDD）/B3（TLA+）执行者时，**将该报告随 IR 一并注入**：BDD 用它划 Feature 边界与端到端功能链，TLA+ 用它做 L1→L2→L3 层次化 module 拆解（见 `executor-bdd.md`/`executor-tlaplus.md` 的「原子操作树」章节）。报告为 warning 级辅助：`wellFormed: true` 可放心按树的层次组织；`uncoveredRequirements`/`emptyLeafSubsystems` 非空时执行者须回报而非臆断补全。注：R3 已通过意味着 `atomicTree` 通常良构（`checkAtomicTree` 门禁），若因跳过（无 architecture 节点）而缺失，B2/B3 退化为按现有 IR 边组织。
