...
- **RID↔IR 映射（§P1-2）**：冻结资产用 `@RID-BDD-*` 标签，本技能产物用 `R1-Sxxx`。生成前先运行 `build-rid-mapping --frozen frozen/features --workdir .srs_formalizer` 产出 `_ctx/rid_mapping.json`，作为需求→BDD 追溯主键；BDD 生成时优先复用冻结的 `RID-BDD-*` 标签而非新造 `R1-Sxxx`。
...
加载提示词 `prompts/executor-backend-traceability.md`，Agent 构建 V-Model 追溯矩阵，将 IR 需求节点与 BDD/TLA+/Lean/fixture 产物交叉引用，输出 `outputs/reports/traceability.md` 与 `traceability.cypher`。追溯以 `_ctx/rid_mapping.json` 的 RID 为主键（§P1-2）。
...
- 该分析依赖 `_ctx/rid_mapping.json`（§P1-2）建立需求↔场景链；缺失时先运行 `build-rid-mapping`。
...
| RID↔IR 映射 | `_ctx/rid_mapping.json` | `build-rid-mapping --frozen <dir>`（§P1-2） |
