...
**P2-2 语境判定规则**（字面匹配过宽会误判章节标题为 NFR）：
- **标题行命中权重 0.3**：Markdown `#`/`##`/`###` 开头的行，或 HTML `<h1>`~`<h6>` 内的命中
- **陈述句命中权重 1.0**：含规范性情态动词（must/须/应/不得/禁止/shall/should）的句子中的命中
- **普通描述命中权重 0.5**：非标题、非规范句的命中
- 同一关键词在一行内多次出现按 1 次计（避免重复计数）

`nfr_weight` = Σ(各类命中权重) / max(1, shard 行数) 归一化到 0~1。
