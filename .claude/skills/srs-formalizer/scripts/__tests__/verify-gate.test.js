...
describe('verify-gate command', () => {
    ...
    it('R3 stage: fails when r2-implicit directory is missing', async () => {
        ...
        assert.ok(errors.some((e) => e.includes('r2-implicit') || e.includes('Some subdirectories')));
    });
    it('R3 stage: fails when duplicate IDs exist across files', async () => {
        ...
        assert.ok(data.errors.some((e) => e.includes('Duplicate')));
    });
...
//# sourceMappingURL=verify-gate.test.js.map