...
describe('validate-cypher command', () => {
  ...
  function writeCypher(fileName: string, content: string): string {
  ...
  it('rejects empty file', async () => {
    ...
    assert.ok((result.data as ValidationResultData).errors.some((e: string) => e.includes('empty')));
  });
  ...
  it('rejects file without CREATE or MATCH statement', async () => {
    ...
    assert.ok(errors.some((e: string) => e.includes('CREATE') || e.includes('MATCH')));
  });
  ...
  it('detects missing semicolon on CREATE/MATCH line', async () => {
    ...
    assert.equal((result.data as ValidationResultData).valid, false);
    const errors = (result.data as ValidationResultData).errors;
    assert.ok(errors.some((e: string) => e.includes(';')));
  });
  ...
  it('detects unclosed single quote', async () => {
    ...
    assert.equal((result.data as ValidationResultData).valid, false);
    const errors = (result.data as ValidationResultData).errors;
    assert.ok(errors.some((e: string) => e.includes('quote')));
  });
  ...
  it('detects mismatched parentheses', async () => {
    ...
    assert.equal((result.data as ValidationResultData).valid, false);
    const errors = (result.data as ValidationResultData).errors;
    assert.ok(errors.some((e: string) => e.includes('bracket') || e.includes('opening')));
  });
