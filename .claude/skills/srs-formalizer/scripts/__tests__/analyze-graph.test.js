...
describe('analyze-graph command', () => {
    ...
    it('detects duplicate requirements via Jaccard similarity (>0.7)', async () => {
        ...
        assert.ok(data.duplicate_pairs >= 1);
        ...
        const dups = readJsonlRecords(dupPath);
    ...
    it('detects antonym pairs (conflicts) from negation patterns', async () => {
        ...
        assert.equal(result.status, 'ok');
        const data = result.data;
        assert.ok(data.conflict_pairs >= 1);
        ...
        const conflicts = readJsonlRecords(conPath);
    ...
    it('detects same-aspect clusters from shared conceptual objects', async () => {
        ...
        assert.equal(result.status, 'ok');
        const data = result.data;
        assert.ok(data.aspect_clusters >= 1);
        ...
        const clusters = readJsonlRecords(aspPath);
...
//# sourceMappingURL=analyze-graph.test.js.map