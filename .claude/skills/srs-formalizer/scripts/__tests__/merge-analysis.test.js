 ...
 */
function writeFixedGraph(workDir, data) {
...
describe('merge-analysis command', () => {
    ...
    it('applies duplicate verdict — merges nodeB into nodeA', async () => {
        ...
        const mergedGraph = JSON.parse(fs.readFileSync(mergedPath, 'utf-8'));
        ...
        const log = readJsonlRecords(logPath);
    ...
    it('applies conflict verdict — adds :CONFLICTS_WITH edge', async () => {
        ...
        assert.ok((conflictEdge.source === 'R1-REQ-0001' && conflictEdge.target === 'R1-REQ-0002') ||
            (conflictEdge.source === 'R1-REQ-0002' && conflictEdge.target === 'R1-REQ-0001'), 'Conflict edge should connect R1-REQ-0001 and R1-REQ-0002');
    });
    // -----------------------------------------------------------------------
    it('applies same_aspect verdict — adds :SAME_ASPECT edges between cluster nodes', async () => {
        ...
        assert.ok((aspectEdge.source === 'R1-REQ-0001' && aspectEdge.target === 'R1-REQ-0002') ||
            (aspectEdge.source === 'R1-REQ-0002' && aspectEdge.target === 'R1-REQ-0001'), 'Same-aspect edge should connect R1-REQ-0001 and R1-REQ-0002');
    });
    // -----------------------------------------------------------------------
...
//# sourceMappingURL=merge-analysis.test.js.map