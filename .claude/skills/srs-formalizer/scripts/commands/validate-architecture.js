 ...
 */
function detectArchLevel(record) {
    if (typeof record.id === 'string') {
        if (record.id.startsWith('ARCH2-'))
            return 2;
        if (record.id.startsWith('ARCH3-'))
            return 3;
        if (record.id.startsWith('ARCH-'))
            return 1;
    }
    return null;
}
        ...
        if (!isArray(record.contains)) {
            ...
            for (let ci = 0; ci < record.contains.length; ci++) {
                ...
                if (!isString(ref)) {
                    ...
                    errors.push(`${prefix}: contains[${ci}] "${ref}" does not match required format R[12]-XXX-NNNN`);
                }
            }
        }
    }
 ...
 */
function crossRecordChecks(records) {
    const errors = [];
    ...
    for (let i = 0; i < records.length; i++) {
        ...
        if (rec.parent !== undefined && rec.parent !== null) {
            if (!isString(rec.parent)) {
                ...
                errors.push(`${prefix}: parent "${rec.parent}" not found in any record's name field`);
            }
        }
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        try {
            ...
            return {
                ...
                data: {
                    valid: false,
                    errors: [`JSON parse error at line ${i + 1}: ${err.message}`],
                    warnings: [],
                    record_count: 0,
                },
            };
