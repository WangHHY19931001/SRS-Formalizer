 ...
 */
function countUnclosedQuotes(line) {
    let single = false;
    let double = false;
    let escaped = false;
...
function checkBrackets(line) {
    ...
    for (const ch of line) {
        if (ch === '(' || ch === '[' || ch === '{') {
            ...
            depth--;
            if (depth < 0)
                return depth; // negative = unmatched closing
        }
    }
 ...
 */
function validateCypher(content) {
    const errors = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inSingleQuote && !inDoubleQuote) {
            if (quoteState.single > 0)
                inSingleQuote = true;
            if (quoteState.double > 0)
                inDoubleQuote = true;
        }
        else {
            if (inSingleQuote) {
...
//# sourceMappingURL=validate-cypher.js.map