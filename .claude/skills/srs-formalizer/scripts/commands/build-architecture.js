 ...
 */
function ensureModuleNode(graph, name, nameMap) {
 ...
 */
function graphHasEdge(graph, edgeId) {
 ...
 */
function processArch1(graph, records, metrics) {
    // Pass 1: create all nodes
    for (const rec of records) {
        if (graph.hasNode(rec.id))
            continue;
        const baseProps = {
            name: rec.name,
 ...
 */
function processArch2(graph, records, metrics) {
    const nameMap = buildNameMap(graph);
    for (const rec of records) {
        switch (rec.action) {
            // -------------------------------------------------------------------
            ...
            case 'reparent': {
                ...
                const filteredEdges = currentData.edges.filter(e => !(e.type === ':PARENT_OF' && e.target === targetId));
 ...
 */
function processArch3(graph, records) {
    for (const rec of records) {
        switch (rec.action) {
            // -------------------------------------------------------------------
...
export async function main(args) {
    ...
    const metrics = { modules: 0, actors: 0, constraints: 0, contains_edges: 0 };
    ...
    if (fs.existsSync(arch1Path)) {
        const records = readJsonLines(arch1Path);
    ...
    if (fs.existsSync(arch2Path)) {
        const records = readJsonLines(arch2Path);
    ...
    if (fs.existsSync(arch3Path)) {
        const records = readJsonLines(arch3Path);
...
//# sourceMappingURL=build-architecture.js.map