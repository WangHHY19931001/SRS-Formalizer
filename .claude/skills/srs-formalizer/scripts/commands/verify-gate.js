...
const VALID_STAGES = ['S1', 'R3', 'FINAL'];
// ---------------------------------------------------------------------------
// Check functions
// ---------------------------------------------------------------------------
function checkStateMd(workDir) {
...
function checkGraphLoadable(workDir) {
    ...
    try {
        ...
        return {
            name: 'Graph loadable',
            passed: false,
            detail: `Failed to load graph: ${err.message}`,
        };
    }
}
function checkNodeCountVsR1(workDir) {
    ...
    try {
        ...
        return {
            name: 'Node count >= R1 explicit requirements',
            passed: false,
            detail: `Cannot check: ${err.message}`,
        };
    }
...
function checkValidateBddPasses(workDir) {
    ...
    try {
        ...
        for (const fileName of featureFiles) {
            ...
            if (!hasFeature)
                errors.push(`${fileName}: missing Feature: line`);
            if (!hasScenario)
                errors.push(`${fileName}: missing Scenario: line`);
            if (!hasSteps)
                errors.push(`${fileName}: missing step definitions (Given/When/Then)`);
        }
        if (errors.length > 0) {
            return {
                name: 'validate-bdd passes',
        ...
        return {
            name: 'validate-bdd passes',
            passed: false,
            detail: `BDD validation error: ${err.message}`,
        };
    }
}
...
function checkMindmapModules(workDir) {
...
export async function main(args) {
    ...
    if (!VALID_STAGES.includes(stageArg)) {
        return {
            status: 'error',
