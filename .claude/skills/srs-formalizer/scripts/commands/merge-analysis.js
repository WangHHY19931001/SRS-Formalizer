 ...
 */
function applyMergeNodes(graph, nodeA, nodeB) {
    ...
    const updatedEdges = currentData.edges.map(e => {
        let modified = false;
 ...
 */
function applyAddConflictEdge(graph, nodeA, nodeB, reasoning) {
    if (!graph.hasNode(nodeA) || !graph.hasNode(nodeB)) {
    ...
    const alreadyExists = existingEdges.some(e => (e.source === nodeA && e.target === nodeB && e.type === ':CONFLICTS_WITH') ||
 ...
 */
function applyAddSameAspectEdge(graph, nodeA, nodeB, reasoning) {
    if (!graph.hasNode(nodeA) || !graph.hasNode(nodeB)) {
        throw new Error(`Cannot add same-aspect edge: node(s) not found (${nodeA}, ${nodeB})`);
    }
    const edgeId = `${nodeA}--:SAME_ASPECT--${nodeB}`;
    const existingEdges = graph.getAllEdges();
    const alreadyExists = existingEdges.some(e => (e.source === nodeA && e.target === nodeB && e.type === ':SAME_ASPECT') ||
...
export async function main(args) {
    ...
    for (const rec of readJsonlRecords(path.join(analysisDir, 'suspected_duplicates.jsonl'))) {
        duplicateLookup.set(rec.pairId, rec);
    }
    // Load suspected_conflicts.jsonl
    for (const rec of readJsonlRecords(path.join(analysisDir, 'suspected_conflicts.jsonl'))) {
        conflictLookup.set(rec.pairId, rec);
    }
    // Load same_aspect_clusters.jsonl
    for (const rec of readJsonlRecords(path.join(analysisDir, 'same_aspect_clusters.jsonl'))) {
...
//# sourceMappingURL=merge-analysis.js.map