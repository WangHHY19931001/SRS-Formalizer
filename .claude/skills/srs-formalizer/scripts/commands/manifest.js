...
function identifyChapters(content) {
    const chapters = [];
...
function shardContent(content, chapters, lang) {
    const shardEntries = [];
...
function detectGaps(content, chapters) {
    const gaps = [];
...
function generateContext(shards, gaps, content) {
...
export async function main(args) {
    ...
    const lang = (parseArg(args, '--lang') || 'zh');
...
${gaps.map((g, i) => `| GAP-${String(i + 1).padStart(3, '0')} | ${g.priority} | ${g.type} | ${g.description} | ${g.source_chapter} | — | — | 待处理 |`).join('\n')}
