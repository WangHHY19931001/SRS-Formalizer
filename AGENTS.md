...
- `verify-gate --stage R3` adds three convergence gates: a **hierarchy-depth gate** (architecture `contains` chain depth ≥2; ≥3 arch nodes with no hierarchy = `flatTree` fail), an **orphan-adjudication gate** (each orphan shard must be declared standalone with a reason in `_ctx/orphan_adjudications.json` or have an accepted bridge), and an **atomic-operation-tree gate** (`checkAtomicTree`). `check-connectivity` reports `hierarchyDepth`/`flatTree`/`architectureNodes` plus an `atomicTree` report (`analyzeAtomicTree`).
- **Atomic-operation tree** (modeling-completeness gate): the `contains` relation must form a single-root reachable spanning tree rooted at the top-level system, with subsystems expanded layer by layer and atomic requirements attached at the leaves. `checkAtomicTree` fails on any of: not exactly one root, a `contains` cycle, an unreachable (floating) subsystem, an empty leaf subsystem (no atomic requirement attached via `contains`/`refines`/`traces_to`/`implements`/`derived_from`), or an uncovered requirement. This is also the static skeleton of a multi-layer FSM abstraction (architecture node = sub-FSM, `contains` = hierarchical refinement, leaf = atomic-operation FSM); a well-formed tree is injected into Backend B2/B3 to drive BDD Feature boundaries and TLA+ L1→L2→L3 module decomposition. Skipped when there are no architecture nodes.
